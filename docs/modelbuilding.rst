...
unknown variables to data. Models are represented by the ``Model`` class. PyMC provides three basic building blocks for Bayesian probability models: 

    ``FreeRV`` - variable whose value is not completely determined by its parents and not observed
    ``ObservedRV`` - variable whose value is not completely determined by its parents and not observed
    ``Deterministic`` - variable 
that is entirely determined by its parents
    ``Potential`` - represents 'factor potentials' 
...
distributions to modify them.

A Model contains a number of these building blocks linked together.

These objects have very limited awareness of the models in which they are embedded and do not themselves possess methods for updating their values in fitting algorithms. Objects responsible for fitting probability models are described in chapter :ref:`chap_modelfitting`.
   
Creation of stochastic variables
--------------------------------

There are two main ways to create stochastic variables, called the 
**automatic**, _____________________________

**Automatic**
   Stochastic variables with standard distributions provided by PyMC (see 
   chapter :ref:`chap_distributions`) can be created in a single line using that distribution's constructor by passing a name for the random variable as the first argument. For example, the uniformly-distributed 
   ...
   The available distributions are described in the chapter :ref:`chap_distributions`.
